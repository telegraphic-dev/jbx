...
- `jbx search <text|group:artifact[:version]> [--limit N] [--json]` search Maven Central artifacts
...
- `jbx search <text|group:artifact[:version]>` queries Maven Central Search; `--json` returns structured `query`, `numFound`, and `artifacts` for agents
